...
echo "[gate] build scan: minikube image"
BUILD_ENV="$(python3 - "$SPEC" "$REPORT_DIR" <<'PY'
import json
import re
import sys
from pathlib import Path
...
workspace = spec.get("workspace", {})
app_root = Path(workspace.get("path", spec_path.resolve().parents[1])).resolve()
dockerfile_path = (app_root / workspace.get("dockerfile_path", "Dockerfile")).resolve()
release_id = str(spec["change_id"])
service = str(spec["service"])
image_repository = slug(service)
image_tag = slug(release_id)
payload = {
    "app_root": str(app_root),
    "dockerfile_path": str(dockerfile_path),
    "image_repository": image_repository,
    "image_tag": image_tag,
    "full_image": f"{image_repository}:{image_tag}",
    "artifact_json": str(report_dir / "docker-image-artifact.json"),
    "artifact_txt": str(report_dir / "docker-image-tag.txt"),
}
...
if [ ! -f "$dockerfile_path" ]; then
  echo "[gate] minikube image build failed: Dockerfile not found at $dockerfile_path"
  exit 1
fi

if ! command -v minikube >/dev/null 2>&1; then
  echo "[gate] minikube image build failed: minikube CLI not found"
  exit 1
fi

if ! minikube status >/dev/null 2>&1; then
  echo "[gate] minikube image build failed: minikube is not running"
  exit 1
fi

echo "[gate] minikube image build image=$full_image"
minikube image build -f "$dockerfile_path" -t "$full_image" .

if ! minikube image ls --format=json | python3 - "$full_image" <<'PY'
import json
import sys
...
PY
then
  echo "[gate] minikube image build failed: image not found after build: $full_image"
  exit 1
fi

mkdir -p "$REPORT_DIR"
python3 - "$artifact_json" "$artifact_txt" "$SPEC" "$app_root" "$dockerfile_path" "$image_repository" "$image_tag" <<'PY'
import json
import sys
from pathlib import Path

artifact_json = Path(sys.argv[1])
artifact_txt = Path(sys.argv[2])
spec_path = sys.argv[3]
app_root = sys.argv[4]
dockerfile_path = sys.argv[5]
image_repository = sys.argv[6]
...
artifact_txt.write_text(f"{image_repository}:{image_tag}\n", encoding="utf-8")
PY
echo "[gate] docker image artifact=$artifact_json"
